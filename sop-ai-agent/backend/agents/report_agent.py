...
def run_report(user_message: str, history_str: str = "") -> dict:
    ...
    raw_data_context = run_data_query(user_message, history_str)
    ...
    system_prompt = prompt_text.replace("{raw_data}", raw_data_context).replace("{history}", history_str)
    ...
    try:
        ...
        logger.error(f"Error parsing JSON from report LLM: {e}")
        return {
